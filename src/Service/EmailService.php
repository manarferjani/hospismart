...
class EmailService
{
    public function sendStockAlert(string $to, string $medicamentName, int $quantity, int $threshold, ?string $imageUrl = null): void
    {
        ...
        try {
            ...
            if ($imageUrl && file_exists($imageUrl)) {
                $mail->addEmbeddedImage($imageUrl, 'medicament_img');
                $cidImage = 'cid:medicament_img';
            }
            ...
            $mail->Body = $this->buildEmailBody(
                ...
                $percentage, $date, $refNumber, $cidImage
            );
    ...
    private function buildEmailBody(
        ...
        ?string $cidImage = null
    ): string {
        $progressBarColor = $alertColor;
        $progressWidth = min($percentage, 100);

        // Build medication card with optional image
        $medicamentCard = '<table role="presentation" width="100%" cellpadding="0" cellspacing="0">';
        $medicamentCard .= '<tr>';
        
        if ($cidImage) {
            $medicamentCard .= <<<HTML
                <td width="120" style="padding-right: 20px; vertical-align: middle;">
                    <img src="{$cidImage}" alt="{$medicamentName}" style="width: 100px; height: 100px; border-radius: 10px; object-fit: cover; border: 3px solid #ffffff; box-shadow: 0 2px 8px rgba(0,0,0,0.1);" />
                </td>
HTML;
        }

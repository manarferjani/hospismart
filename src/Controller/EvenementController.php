...
class EvenementController extends AbstractController
{
    ...
    /**
     * MES ÉVÉNEMENTS - Événements auxquels l'utilisateur s'est inscrit
     */
    #[Route('/mes-evenements', name: 'app_mes_evenements', methods: ['GET'])]
    public function mesEvenements(EntityManagerInterface $entityManager, WeatherService $weatherService): Response {
        /** @var User $user */
        $user = $this->getUser();

        if (!$user) {
            $this->addFlash('warning', 'Vous devez être connecté pour voir vos événements.');
            return $this->redirectToRoute('app_login');
        }

        // Récupérer les participations de l'utilisateur
        $participations = $entityManager->getRepository(ParticipantEvenement::class)->findBy(
            ['participant' => $user],
            ['dateInscription' => 'DESC']
        );

        // Extraire les événements
        $evenements = array_map(fn($p) => $p->getEvenement(), $participations);
        $weatherData = $weatherService->getForecastsForEvents($evenements);

        return $this->render('front/mes_evenements.html.twig', [
            'evenements' => $evenements,
     ...
     */
    #[Route('/evenement/desinscription/{id}', name: 'app_evenement_desinscription', methods: ['POST'])]
    public function desinscription(ParticipantEvenement $participation, Request $request, EntityManagerInterface $entityManager): Response {
        /** @var User $user */
        $user = $this->getUser();
        ...
        if ($participation->getParticipant()->getId() !== $user->getId()) {
            $this->addFlash('danger', 'Action non autorisée.');
            return $this->redirectToRoute('app_mes_evenements');
        }

        if ($this->isCsrfTokenValid('delete'.$participation->getId(), $request->request->get('_token'))) {
            $evenementTitre = $participation->getEvenement()->getTitre();
            $entityManager->remove($participation);
            $entityManager->flush();
            $this->addFlash('success', 'Vous vous êtes désinscrit de l\'événement "' . $evenementTitre . '".');
        }

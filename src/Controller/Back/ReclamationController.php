...
class ReclamationController extends AbstractController
{
    ...
    public function show(Request $request, Reclamation $reclamation, EntityManagerInterface $em): Response
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            $reponse->setAdminEmail($user ? ($user->getEmail() ?? 'admin@hospismart.com') : 'admin@hospismart.com');
    ...
    public function editResponse(Request $request, Reponse $reponse, EntityManagerInterface $em): Response
    {
        ...
        if ($content) {
            $reponse->setContenu((string) $content);
            $em->flush();
            $this->addFlash('success', 'Réponse modifiée.');
        }
        $reclamation = $reponse->getReclamation();
        return $this->redirectToRoute('app_back_reclamation_show', ['id' => $reclamation ? $reclamation->getId() : 0]);
    }
    ...
    public function deleteResponse(Request $request, Reponse $reponse, EntityManagerInterface $em): Response
    {
        if ($this->isCsrfTokenValid('delete'.$reponse->getId(), (string) $request->request->get('_token'))) {
            $reclamation = $reponse->getReclamation();
            $reclamationId = $reclamation ? $reclamation->getId() : 0;
            $em->remove($reponse);
            $em->flush();

...
class UtilisateurCrudController extends AbstractController
{
    ...
    public function new(
        ...
        if ($request->isMethod('POST')) {
            ...
            // Définir le type utilisateur basé sur le rôle
            $type = match($role) {
            ...
            $user->setGenre($genre !== null ? (string) $genre : null);
            if ($dateNaissanceStr) {
                try {
                    $user->setDateNaissance(new \DateTime((string) $dateNaissanceStr));
                } catch (\Exception $e) {}
            }
            $user->setGroupeSanguin($groupeSanguin !== null ? (string) $groupeSanguin : null);
            $user->setAdresse($adresse !== null ? (string) $adresse : null);
            
            $user->setSpecialite($specialite !== null ? (string) $specialite : null);
            $user->setMatricule($matricule !== null ? (string) $matricule : null);
    ...
    public function edit(
        ...
        if ($request->isMethod('POST')) {
            $user->setNom((string) $request->request->get('nom', $user->getNom()));
            $user->setPrenom((string) $request->request->get('prenom', $user->getPrenom()));
            $user->setEmail((string) $request->request->get('email', $user->getEmail()));
            $user->setTelephone($request->request->get('telephone') ? (string) $request->request->get('telephone') : null);
            ...
            // Champs Patient
            $user->setGenre((string) $request->request->get('genre', $user->getGenre()));
            if ($request->request->get('date_naissance')) {
                $user->setDateNaissance(new \DateTime((string) $request->request->get('date_naissance')));
            }
            $user->setGroupeSanguin((string) $request->request->get('groupe_sanguin', $user->getGroupeSanguin()));
            $user->setAdresse((string) $request->request->get('adresse', $user->getAdresse()));

            // Champs Medecin
            $user->setSpecialite((string) $request->request->get('specialite', $user->getSpecialite()));
            $user->setMatricule((string) $request->request->get('matricule', $user->getMatricule()));
            // Le téléphone médecin (user_telephone dans le form) est le même que le user telephone
            if ($request->request->get('user_telephone')) {
                 $user->setTelephone((string) $request->request->get('user_telephone'));
            }

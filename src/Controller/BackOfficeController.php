...
class BackOfficeController extends AbstractController
{
    ...
    public function dashboard(Request $request, ReclamationRepository $reclamationRepository, PaginatorInterface $paginator): Response
    {
        ...
        /** @var Reclamation[] $allReclamations */
    ...
    #[Route('/reclamation/{id}', name: 'back_office_voir_reclamation', methods: ['GET'])]
    public function voirReclamation(Reclamation $reclamation, AuditLogRepository $auditLogRepository): Response
    {
        $auditLogs = $auditLogRepository->findByReclamation((int) $reclamation->getId());
    ...
    public function repondreReclamation(Request $request, Reclamation $reclamation, EntityManagerInterface $entityManager, ValidatorInterface $validator, AuditService $auditService): Response
    {
        $reponse = new Reponse();
        ...
        if ($form->isSubmitted()) {
            ...
            if (count($errors) > 0 || !$form->isValid()) {
                ...
                return $this->render('back_office/repondre_reclamation.html.twig', [
                    'reclamation' => $reclamation,
            ...
            $auditService->logReclamationResponded((int) $reclamation->getId(), (string) $reponse->getContenu(), $reponse->getAdminNom());
            
            // Log the status change to Traité with the actual old status
            $auditService->logStatusChanged((int) $reclamation->getId(), (string) $oldStatus, 'Traité', $reponse->getAdminNom());

            $this->addFlash('success', 'Réponse envoyée avec succès et réclamation marquée comme traitée !');
    ...
    public function supprimerReclamation(Request $request, Reclamation $reclamation, EntityManagerInterface $entityManager, AuditService $auditService): Response
    {
        ...
        try {
            ...
            $auditService->logReclamationDeleted((int) $reclamationId, (string) $reclamationTitre);

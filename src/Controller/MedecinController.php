...
final class MedecinController extends AbstractController
{
    ...
    public function editDispo(int $id, Request $request, EntityManagerInterface $em): Response
    {
        ...
        $dateDebut = new \DateTime((string) $request->request->get('date_debut'));
        $dateFin = new \DateTime((string) $request->request->get('date_fin'));
...
public function modifierDate(RendezVous $rdv, Request $request, EntityManagerInterface $em): Response
{
    ...
    if ($nouvelleDateStr) {
        $nouvelleDate = new \DateTime((string) $nouvelleDateStr);
        ...
        // createdAt et isRead sont déjà gérés par le __construct
        $notification->setIsRead(false);
